...
Weâ€™re introducing scheduled tasks, allowing users to plan ahead with their projects. Utilizing the newest Gemini 3.5 Flash model, users can schedule messages to be sent to their agents while theyâ€™re away.  

* **Repeatable**: Set up time-based triggers to start conversations periodically.  
* Tasks will be set to repeat on the minute youâ€™ve set them.
...
* **Bounded Access**: By default, your agent can only read and write within the provided folders of a project. If you change your security preset to â€œFull Machineâ€ or â€œUnrestrictedâ€, the agent will have read and write access over your full machine.
...
* **Shortcut**: You can start recording by pressing `Ctrl + M`. Once youâ€™re done, press `Ctrl + M` to stop recording.
...
JSON Hooks allow you to execute custom local shell scripts at critical stages of an Antigravity agent's execution cycle. You can intercept and control the agent's behavior before tool calls, after model responses, or at loop stopping conditionsâ€”configured globally or per-workspace via simple JSON files.
