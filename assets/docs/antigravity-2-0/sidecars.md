...
Each sidecar has its own directory and the directory name is used as the sidecarâ€™s ID. Sidecars loaded from plugins have the ID `<pluginName>/<sidecarName>`. 

The sidecarâ€™s directory must contain a `sidecar.json` file and may also contain other helper files like scripts to run. The sidecarâ€™s directory also acts as the current working directory for the sidecarâ€™s command.
...
Sidecars can use the `agentapi` CLI to programmatically interact with Antigravity. The executable is automatically added to the sidecarâ€™s path and available as `agentapi`.
